...
<head>
    ...
    <title>{!! !empty($title) ? $title : 'Ethiopika | jobs in Oromia' !!}</title>
...
<div id="app">
    ...
    <nav class="navbar navbar-expand-md navbar-light navbar-full navbar-laravel {{request()->routeIs('home') ? 'transparent-navbar' : ''}}"   >
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                ...
                <ul class="navbar-nav mr-auto">
                    <li class="nav-item " ><a class="nav-link link2"  href="{{route('home')}}" ><i class="la la-home"></i> @lang('app.home')</a> </li>

                   

                    <li class="nav-item"><a class="nav-link link2" href="{{route('jobs_listing')}}"><i class="la la-briefcase"></i> @lang('app.jobs')</a> </li>
                    <li class="nav-item"><a class="nav-link link2" href="{{route('pricing')}}"><i class="la la-dollar"></i> @lang('app.pricing')</a> </li>
                    <li class="nav-item"><a class="nav-link link2" href="{{route('contact_us')}}"><i class="la la-envelope-o"></i> @lang('app.contact_us')</a> </li>
                ...
                <ul class="navbar-nav ml-auto">
                    ...
                    @guest
                        <li class="nav-item">
                            <a class="nav-link link2" href="{{ route('login') }}"><i class="la la-sign-in"></i> {{ __('app.login') }}</a>
                        </li>
                        <li class="nav-item">
                            @if (Route::has('new_register'))
                                <a class="nav-link link2" href="{{ route('new_register') }}"><i class="la la-user-plus"></i> {{ __('app.register') }}</a>
                            @endif
                        </li>
                    @else
                ...
                <div class="col-md-4">
                    ...
                    @if($header_menu_pages->count() > 0)
                        @foreach($header_menu_pages as $page)
                            <li><a href="{{ route('single_page', $page->slug) }}"><i class="la la-link"></i>{{ $page->title }} </a></li>
                        @endforeach
                    @endif
